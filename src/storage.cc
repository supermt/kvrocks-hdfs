...
namespace Engine {
  ...
  Status Storage::Open(bool read_only) {
   ...
   metadata_table_opts.block_cache = shared_block_cache ?
                                     shared_block_cache : rocksdb::NewLRUCache(metadata_block_cache_size, -1,
                                                                               false, 0.75);
   ...
   subkey_table_opts.block_cache = shared_block_cache ?
                                   shared_block_cache : rocksdb::NewLRUCache(subkey_block_cache_size, -1, false,
                                                                             0.75);
  ...
  uint64_t Storage::GetTotalSize(const std::string &ns) {
   ...
   rocksdb::DB::SizeApproximationFlags include_both = (rocksdb::DB::SizeApproximationFlags::INCLUDE_FILES |
